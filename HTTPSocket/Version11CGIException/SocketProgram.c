...
void ParsingRequestMessage(int clientSocket, char* requestMessageBuffer, RequestMessage* requestMessage);
int SelectSendState(RequestMessage* requestMessage);
void SendResponseMessage(int clientSocket, int sendMessageState, RequestMessage* requestMessage);
...
void *RunThreadToCommunicationMessage(void *clientSocketAddress)
{
	...
	int messageLength = ParseRequestMessage(clientSocket, receiveRequestMessageBuffer, &requestMessage);
	printf("!! method %s, URI %s\n", requestMessage.requestMethod, requestMessage.requestURI);
...
void SendResponseMessage(int clientSocket, int sendPacketState, RequestMessage* requestMessage)
{
	...
	case 2001:
		...
		if (fork() == 0)
		{
			...
			else
			{
				...
				if (strstr(requestMessage->environmentVariable[4], "multipart/form-data") != NULL)
				{
...
int SelectSendState(RequestMessage* requestMessage)
{
	...
	if (((strcmp(requestMessage->requestMethod, "GET") != 0) && (strcmp(requestMessage->requestMethod, "POST") != 0)) &&
		(requestMessage->requestURI[0] == '/') || ((atoi(requestMessage->environmentVariable[2]) > 1000) && (strcmp(requestMessage->requestMethod, "POST") != 0)))
	{
		sendPacketState = 404;
	}
	else if ((requestMessage->requestURI[0] == '\0') || (strcmp(requestMessage->requestURI, "main") == 0) || (strcmp(requestMessage->requestURI, "SocketProgram.c") == 0))
	{
		char* tempURL = "HelloWorld.html";
		strcpy(requestMessage->requestURI, tempURL);
		sendPacketState = 200;
	}
	else if (((readFilePointer = fopen(requestMessage->requestURI, "r")) == NULL) && ((readFilePointer = fopen(cGIProgram, "r")) == NULL))
	{
		sendPacketState = 400;
	}
...
int ParseRequestMessage(int clientSocket, char* receiveMessageBuffer, RequestMessage* requestMessage)
{
	...
	strcpy(requestMessage->environmentVariable[3], requestMessage->requestMethod);
	for (j = 0; j < i; j++)
	{
		printf("!%d! %s\n",j,tempRequestMessageLine[j]);
		...
		else if (strstr(tempRequestMessageLine[j], "Content-Length:") != NULL)
		{
			int k = 0;
			int flag = 0;
			int m = 0;
			...
			sprintf(requestMessage->environmentVariable[2], "%d", atoi(tempBuffer));
		}
	}
	strcpy(requestMessage->environmentVariable[5], tempRequestMessageLine[i - 1]);
	////// putting commands
	...
	requestMessage->requestBody = (char *)malloc(sizeof(char)* atoi(requestMessage->environmentVariable[2]));

	memset(requestMessage->requestBody, '\0', atoi(requestMessage->environmentVariable[2]));
	strcpy(tempMessageBuffer, receiveMessageBuffer);
	
	if (strstr(tempMessageBuffer, "------") != NULL)
	{
		strncpy(requestMessage->requestBody,strstr(tempMessageBuffer,"-----"),1000);
		...
		{
			j += recv(clientSocket, requestMessage->requestBody + j, 1024, 0);
		}
	}
	else if(strstr(requestMessage->environmentVariable[4], "multipart/form-data") != NULL)
	{
		j = 0;
