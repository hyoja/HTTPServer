...
void SendResponseMessage(int clientSocket, int sendPacketState, char* requestURL, char environmentVariable[][1024])
{
	...
	switch(sendPacketState)	
	{
		...
		case 2001:
			...
			strcpy(tempp,aa);
			printf("!! before %d of URI NUmber is %s \n",clientSocket,getenv("REQUEST_URI"));
			for(jj = 0 ; jj < 1024;jj++)
			{
				if(tempp[jj] == '\0')	
				{
			...
			if(fork()==0)
			{
					...
					setenv("REQUEST_URI",tempp,1); // name=value&name1=value*/
					//system("./cgi-bin/calc.cgi");
					//close(pipeFileDescriptor[0]);
					close(pipeFileDescriptor[0]);
					//write(pipeFileDescriptor[1],"hhahaha",BUFFERSIZE);
...
int ParseRequestMessage(int clientSocket, char* receiveMessageBuffer,char* requestURL,char* requestMethod,char environmentVariable[][1024])
{
	...
	char tempSavedMethod[1024];
	memset(tempSavedMethod,'\0',1024);
...
////// pars protocol commands
	...
	if(whileCount > 1)
	{
		...
		strcpy(tempSavedMethod,requestMethod);
	}
...
///////pars environment variable
	...
	strcpy(requestMethod,tempSavedMethod);
	printf("#1 methode %s, uli %s\n",requestMethod, requestURL);
