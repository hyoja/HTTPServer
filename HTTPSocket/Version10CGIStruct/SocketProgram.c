...
typedef struct requestMessage
{
	...
	char requestURI[100];
	char requestEnviroment[10][1024];
}RequestMessage;
...
void SendResponseMessage(int clientSocket, int sendPacketState, char* requestURL, char environmentVariable[][BUFFERSIZE])
{
	...
	char* dynamicBuffer = (char *)malloc(sizeof(char) * atoi(environmentVariable[2]));
	memset(dynamicBuffer,'\0',atoi(environmentVariable[2]));
	int leng = atoi(environmentVariable[2]);
	printf("leng : %d\n",atoi(environmentVariable[2]));
	...
	case 2001:
		...
		if(strstr(environmentVariable[4],"multipart/form-data") != NULL)
		{
			i = 0;
			while(i < atoi(environmentVariable[2]))
			{
				i += recv(clientSocket, dynamicBuffer + i, 1024, 0);
			}
			printf("%s : env leng %d,, get leng %d,\n",dynamicBuffer,atoi(environmentVariable[2]),i);
		}
		...
		if(fork() ==0)
		{
			...
			else
			{
				...
				if(strstr(environmentVariable[4],"multipart/form-data") != NULL)
				{
					...
					while(1)
					{
						if(i > atoi(environmentVariable[2]))
						{
							break;
						}
						sendingTemper[0] = dynamicBuffer[i];
						if(dynamicBuffer[i+1] == '\0')
						{
							sendingTemper[1] = '\0';
						}
						else
						{
							sendingTemper[1] =dynamicBuffer[i+1];
						}
						printf("%s",sendingTemper);
						i+=2;
					}
				}
				else
...
int ParseRequestMessage(int clientSocket, char* receiveMessageBuffer, char* requestURL, char* requestMethod, char(*environmentVariable)[1024])
{
	...
	for (j = 0; j < i; j++)
	{
		...
		{	
			...
			sprintf(environmentVariable[2], "%d",atoi( tempBuffer));
		}

	}
	strcpy(environmentVariable[5], tempRequestMessageLine[i - 1]);
	//sprintf(environmentVariable[2], "%d", strlen(environmentVariable[5]));
	////// putting commands
